...
public class GoalDAO {
    ...
    public static GoalDAO
    getGoalDAOFromID(String goalUUID) {
        // create goal from UUID
        return new GoalDAO(Realm.getDefaultInstance().where(Goal.class).equalTo("id", goalUUID).findFirst());
    }
    ...
    private void calculateRecurringStats() {
        ...
        while (currZDT.isBefore(endZDT)) {
            ...
            RealmResults sessionResult = mResults.where()
                    .between("datetime", Shared.ZDTToDate(currZDT), Shared.ZDTToDate(nextZDT))
            ...
            if(attempt.getClimb().getGrade()>=mGoal.getMingrade()) {  // only count the climb if its > mingrade
                switch (mGoal.getGoalUnit()) {
                    case POINTS:
                        ...
                        mLineEntries.add(new Entry(xValue, attempts.where().lessThanOrEqualTo("datetime", attempt.getDatetime()).sum("grade").intValue()));
                        break;
                    case CLIMBS:
                        mNonRecurringProgress ++;
                        mLineEntries.add(new Entry(xValue, attempts.where().lessThanOrEqualTo("datetime", attempt.getDatetime()).count()));
                        break;
                    case HEIGHT:
                        // TODO: fill this in
                        break;
